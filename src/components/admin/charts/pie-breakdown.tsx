...
export function PieBreakdown({
  ...
  emptyText: string;
  /** Valores em centavos (mostra em reais). */
  money?: boolean;
}) {
  const formatar = (valor: number) => (money ? formatBRL(valor) : formatNumber(valor));
      ...
      <div className="mx-auto size-40">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip formatter={(valor) => [formatar(Number(valor)), money ? 'Valor' : 'Clientes']} />
          </PieChart>
        </ResponsiveContainer>
      </div>
      <ul className="grid gap-1.5">
        {slices.map((fatia, indice) => (
          <li key={fatia.key} className="flex items-center gap-2 text-[13px]">
            ...
            <span className="tabular text-ink-900">{formatar(fatia.value)}</span>
